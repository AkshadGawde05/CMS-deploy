...
export default function ActionTab({ sources, interests, users, onAction }: ActionTabProps) {
  ...
  const fetchEnquiries = useCallback(async () => {
    try {
      ...
      showToast({
        type: 'error',
        title: 'Error',
        message: 'Failed to fetch action enquiries'
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleMoveEnrolled = async (id: string) => {
    try {
      await updateEnquiryStatus(id, 'enrolled', 'Converted to enrollment from action');
      showToast({
        type: 'success',
        title: 'Success',
      ...
      showToast({
        type: 'error',
        title: 'Error',
        message: 'Failed to move to enrolled'
      });
    }
  };

  const handleMoveLost = async (id: string) => {
    try {
      await updateEnquiryStatus(id, 'lost', 'Marked as lost from action');
      showToast({
        type: 'success',
        title: 'Success',
      ...
      showToast({
        type: 'error',
        title: 'Error',
        message: 'Failed to mark as lost'
      });
    }
  };
  ...
  const handleMoveBack = async (id: string) => {
    try {
      await updateEnquiryStatus(id, 'contacted', 'Moved back from action');
      showToast({
        type: 'success',
        title: 'Success',
  ...
  const handleBulkAction = async (targetStatus: 'enrolled' | 'contacted' | 'lost') => {
    if (selectedEnquiries.length === 0) return;
    try {
      for (const id of selectedEnquiries) {
        await updateEnquiryStatus(id, targetStatus, `Bulk moved from action`);
      }
      showToast({
        type: 'success',
        title: 'Success',
        message: `${selectedEnquiries.length} enquiries updated successfully`
      });
      setSelectedEnquiries([]);
      fetchEnquiries();
      onAction?.();
    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-600">Follow up with interested enquiries</p>
        </div>
        ...
        {selectedEnquiries.length > 0 && (
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <div className="flex gap-1 overflow-x-auto">
              <button
                onClick={() => handleBulkAction('enrolled')}
      ...
      <div className="border-b border-gray-200 mb-4 sm:mb-6">
        <nav className="-mb-px flex space-x-8">
          <button
            ...
            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition ${actionType === 'interested'
          ...
          <button
            ...
            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition ${actionType === 'not_interested'
              ? 'border-blue-500 text-blue-600'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
          >
            ...
            {loading ? (
              ...
              enquiries.map((enquiry) => (
                <tr key={enquiry._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${enquiry.status === 'interested'
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        onClick={() => handleMoveEnrolled(enquiry._id)}
                      ...
                      <button
                        onClick={() => handleMoveLost(enquiry._id)}
            ...
            <div key={enquiry._id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition">
              ...
              <div className="space-y-2 mb-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${enquiry.status === 'interested' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                    {enquiry.status.replace('_', ' ')}
                  </span>
                </div>
              ...
              <div className="flex flex-wrap items-center justify-end gap-1 pt-3 border-t border-gray-100">
                ...
                <button onClick={() => handleMoveEnrolled(enquiry._id)} className="p-2 text-green-600 hover:bg-green-50 rounded transition" title="Mark Enrolled"><TrendingUp className="h-4 w-4" /></button>
                <button onClick={() => handleMoveLost(enquiry._id)} className="p-2 text-red-600 hover:bg-red-50 rounded transition" title="Mark Lost"><TrendingDown className="h-4 w-4" /></button>
