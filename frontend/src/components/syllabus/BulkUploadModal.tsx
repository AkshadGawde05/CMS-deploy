                      ...
                      <div key={idx} className="bg-white p-3 rounded border border-green-200">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <p className="font-medium text-gray-900">{item.batch_name}</p>
                            <p className="text-sm text-gray-600 mt-1">
                              📚 {item.course_name} • 📅 {item.academic_year}
                            </p>
                            <p className="text-xs text-gray-500 mt-1">Items: {item.items_count} | Row {item.row}</p>
                          </div>
                    ...
                    {results.failed.map((item: Record<string, unknown>, idx: number) => (
                      <div key={idx} className="bg-white p-3 rounded border border-red-200">
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          {item.data?.['Batch ID'] && `Batch: ${item.data['Batch ID']}`}
                        </p>
                      </div>
                    ))}
                  </div>
