...
import { getAllBatches, updateFeePlan, type FeePlanDTO } from '@/lib/api';
...
export default function EditFeePlanModal({ plan, onClose, onUpdated }: Props) {
  const [batches, setBatches] = useState<BatchOption[]>([]);
  const [batchId, setBatchId] = useState<string>(
    typeof plan.batch_id === 'string' ? plan.batch_id : plan.batch_id?._id || ''
  );
  const [totalAmount, setTotalAmount] = useState<string>(String(plan.total_amount ?? ''));
  const [numInstallments, setNumInstallments] = useState<string>(String(plan.num_installments ?? '1'));
  const [discountTypes, setDiscountTypes] = useState<Array<{ code: string; name: string; discount_percent: number }>>(
    plan.discount_types || [{ code: 'full', name: 'Full Payment', discount_percent: 0 }]
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : 'Failed to update fee plan';
      setError(msg);
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-2xl rounded-lg bg-white shadow-lg max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white px-5 py-4 border-b text-black z-10">
          <h3 className="text-lg font-semibold">Edit Fee Plan</h3>
        </div>
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          <div>
            <label className="block text-sm font-medium text-black mb-1">Batch</label>
            <select
              className="w-full border rounded px-3 py-2 text-sm text-black-700"
              value={batchId}
